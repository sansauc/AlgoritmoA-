...
public class Controlador {
    ...
    public float calcularDistanciaEuclidea(Nodo nodoOrigen, Nodo nodoDestino) { // cambiar a float
        ...
        return (float) Math.sqrt((nodoDestino.getCoordenadaX() - nodoOrigen.getCoordenadaX())
                ^ 2 + (nodoDestino.getCoordenadaY() - nodoOrigen.getCoordenadaY()) ^ 2);
    }
    ...
    public Nodo obtenerNodoConMenorF(List<Nodo> nodosAbiertos) {
        if (nodosAbiertos.size() == 1) {
            ...
            float fAux = 1000000; // se coloca un valor arbitrariamente alto para que se sobrescriba con el valor
    ...
    public void calcularCostoDePasoAlNodo(Nodo nodoActual, Nodo nodoVecino) {
        ...
        float valorSalto = this.getCostoSaltos(nodoActual, nodoVecino); // obtiene el valor del salto
        nodoVecino.setG(valorSalto + nodoActual.getG());

        if (nodoVecino.getG() == 0) {
            nodoVecino.setG(nodoActual.getG() + valorSalto);
            nodoVecino.setF(nodoVecino.getG() + nodoVecino.getH());
            nodoVecino.setPadre(nodoActual);
        }
        if (nodoVecino.getF() > (valorSalto + nodoActual.getG() + nodoVecino.getH())) {
